Deface::Override.new(
  ...
  text: %{
    ...
    <% payment_method = current_store&.payment_methods.active.available_on_front_end.find_by_type("Spree::PaymentMethod::EdenredJunaeb") %>
    <% if payment_method && @ld_client.variation("edenred-junaeb-payment", @ld_user, true) %>
      <% unless current_store&.url.eql?('https://lomiexpress.cl') %>
        <label class="d-block" for="<%= dom_id(payment_method) %>">
          <div class="card-lomi mb-0">
            <div class="d-flex justify-content-between align-items-center px-1">
          ...
          <% if @order.any_item_with_alcohol? %>
            <p class="ml-4 texto-s"><%= Spree.t(:purchases_with_alcohol_not_allowed, payment_method: 'Edenred Junaeb')%></p>
          <% end %>
        </label>
      <% end %>
    <% end %>
